...
class DocumentRequestServices extends StatelessWidget {
  final requestServicesController = Get.put(RequestServicesontroller());

  DocumentRequestServices({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    var language = AppLocalizations.of(context);
    return GetX<RequestServicesontroller>(builder: (controller) {
      return Center(
        child: Column(
  ...
  Widget buildFile(PlatformFile file) {
    ...
    return InkWell(
      onTap: () => {requestServicesController.viewFile(file)},
      child: ListTile(
  ...
  void pickFiles(String? filetype) async {
    if (requestServicesController.result.isNotEmpty) {
      requestServicesController.result[0] =
          (await FilePicker.platform.pickFiles(allowMultiple: true))!;
    } else {
      requestServicesController.result
          .add((await FilePicker.platform.pickFiles(allowMultiple: true))!);
    }

    requestServicesController.files.value =
        requestServicesController.result[0].files.cast<DocumentModel>();
    if (requestServicesController.result == null) return;

    ListView.builder(
        itemCount: requestServicesController.result[0].files.length,
        itemBuilder: (context, index) {
          final file = requestServicesController.result[0].files[index];
          log("++++++++++++++++++++++++$file");
